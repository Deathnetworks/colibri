...
COLI_SYCL_DLLEXPORT int coli_sycl_tensor_upload(ColiSyclTensor **tensor,
                            const void *weights, const float *scales,
                            int fmt, int I, int O, int device);

COLI_SYCL_DLLEXPORT int coli_sycl_matmul(ColiSyclTensor **tensor,
                     float *y, const float *x,
                     const void *weights, const float *scales,
                     int fmt, int S, int I, int O, int device);

COLI_SYCL_DLLEXPORT int coli_sycl_expert_mlp(ColiSyclTensor *gate, ColiSyclTensor *up,
                         ColiSyclTensor *down, float *y, const float *x, int S);

COLI_SYCL_DLLEXPORT int coli_sycl_shared_mlp_w4a16(ColiSyclTensor *gate, ColiSyclTensor *up,
                               ColiSyclTensor *down, float *y,
                               const float *x, int S);

COLI_SYCL_DLLEXPORT int coli_sycl_expert_group(ColiSyclTensor *const *gates,
                           ColiSyclTensor *const *ups,
                           ColiSyclTensor *const *downs,
                           const int *rows, int count,
                           float *y, const float *x);

COLI_SYCL_DLLEXPORT int coli_sycl_attention_absorb(ColiSyclTensor *kv_b,float *ctx,const float *q,
                               const float *latent,const float *rope,int H,int Q,
                               int R,int V,int K,int T,float attention_scale);

COLI_SYCL_DLLEXPORT int coli_sycl_attention_absorb_batch(ColiSyclTensor *kv_b,float *ctx,const float *q,
                                     const float *latent,const float *rope,int S,
                                     int H,int Q,int R,int V,int K,int T,
                                     float attention_scale);

COLI_SYCL_DLLEXPORT int coli_sycl_attention_project_batch(ColiSyclTensor *kv_b,ColiSyclTensor *o_proj,
...
COLI_SYCL_DLLEXPORT void coli_sycl_tensor_free(ColiSyclTensor *tensor);
COLI_SYCL_DLLEXPORT size_t coli_sycl_tensor_bytes(const ColiSyclTensor *tensor);
COLI_SYCL_DLLEXPORT int coli_sycl_tensor_device(const ColiSyclTensor *tensor);
COLI_SYCL_DLLEXPORT int coli_sycl_tensor_update(ColiSyclTensor *tensor,
                            const void *weights, const float *scales);
...
COLI_SYCL_DLLEXPORT int coli_sycl_pipe_gemm(ColiSyclTensor *t,float *y_dev,const float *x_dev,int S);
...
COLI_SYCL_DLLEXPORT int coli_sycl_attention_project_batch_dev(ColiSyclTensor *kv_b,ColiSyclTensor *o_proj,
...
COLI_SYCL_DLLEXPORT int coli_sycl_attention_absorb_batch_dev(ColiSyclTensor *kv_b_shard,float *ctx_dev,
        const float *q_dev,const float *latent_dev,const float *rope_dev,
        int S,int H,int Q,int R,int V,int K,int T,float scale);
COLI_SYCL_DLLEXPORT int coli_sycl_attention_absorb_kvdev(ColiSyclTensor *kv_b,float *ctx,const float *q,
...
COLI_SYCL_DLLEXPORT int coli_sycl_attention_project_batch_dev_out(ColiSyclTensor *kv_b,ColiSyclTensor *o_proj,
